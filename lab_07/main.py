    ...
    with load_default() as loaders:
        for load in loaders:
            ...
            for size in cell_sizes:
                res.append(get_d(load, size, 2))

            ax.plot(cell_sizes, res)

        ax.set(xlabel='Cell size', ylabel='Minkowski dimension')
        ax.grid()
        ax.legend([load.name for load in loaders])
        fig.savefig("./out/plt.png")
